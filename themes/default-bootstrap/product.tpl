...
{if $errors|@count == 0}
	...
	{/if}
<div itemscope itemtype="https://schema.org/Product" class="b1c-good">
		...
		<div class="pb-center-column col-xs-12 col-sm-5">
			...
			<div class="product-features">
				<ul class="product-features__list">
					<li class="product-features__item{if empty($product->reference) || !$product->reference} hidden{/if}" itemprop="sku"{if !empty($product->reference) && $product->reference} content="{$product->reference}"{/if}><span class="product-features__name">{l s='Reference:'}</span> {$product->reference}</li>
                    {if isset($features) && $features}
                        {foreach from=$features item=feature}
                            <li class="product-features__item"><span class="product-features__name">{$feature.name|escape:'html':'UTF-8'}:</span> {$feature.value|escape:'html':'UTF-8'}</li>
                        {/foreach}
                    {/if}
				</ul>
			</div>
			...
			<form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
				...
				<div class="box-info-product">
					<div class="product_attributes clearfix">
						<!-- quantity wanted -->
                        {if !$PS_CATALOG_MODE}
							<p id="quantity_wanted_p"{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || $PS_CATALOG_MODE} style="display: none;"{/if} class="clearfix">
                                ...
                                {foreach from=$groups key=id_attribute_group item=group}
                                    {if $group.attributes|@count}
										<fieldset class="attribute_fieldset">
											<label class="attribute_label b1c-caption" {if $group.group_type != 'color' && $group.group_type != 'radio'}for="group_{$id_attribute_group|intval}"{/if}>{$group.name|escape:'html':'UTF-8'}&nbsp;</label>
                                            {assign var="groupName" value="group_$id_attribute_group"}
											<div class="attribute_list">
                                                {if ($group.group_type == 'select')}
													<select name="{$groupName}" id="group_{$id_attribute_group|intval}" class="b1c-option form-control attribute_select no-print">
                                                ...
                                                {elseif ($group.group_type == 'radio')}
													<ul>
                                                        {foreach from=$group.attributes key=id_attribute item=group_attribute}
															<li>
																<input type="radio" class="attribute_radio b1c-option" name="{$groupName|escape:'html':'UTF-8'}" value="{$id_attribute}" {if ($group.default == $id_attribute)} checked="checked"{/if} />
					...
					<div class="box-cart-bottom">
						<div{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="unvisible"{/if}>
							<p id="add_to_cart" class="buttons_bottom_block no-print">
								<button type="submit" name="Submit" class="exclusive btn_orange btn">
									<span>{if $content_only && (isset($product->customization_required) && $product->customization_required)}{l s='Customize'}{else}{l s='Add to cart'}{/if}</span>
								</button>
							</p>
							<p id="buyme" class="buttons_bottom_block no-print">
								<button type="button" name="buyme" class="exclusive btn_blue btn b1c">
									<span>{l s="Заказать в один клик"}</span>
								</button>
							</p>
						</div>
...
{*{if (isset($quantity_discounts) && count($quantity_discounts) > 0)}*}
		...
		{if isset($product) && $product->description}
			...
			<section class="page-product-box">
				<!-- full description -->
				<div class="rte rte_dark rte_description">
                    <div class="container">
                        <div class="row">
							<h3 class="rte__title">{l s='More info'}</h3>
                            {$product->description}
                        </div>
                    </div>
				</div>
			...
			<!--end  More info -->
			<div class="container" id="columns"><div class="row"><div class="col-xs-12 col-sm-12"><div>
		{/if}
		{if isset($packItems) && $packItems|@count > 0}
		<section id="blockpack">
			<h3 class="title title_blue"><span class="title__text">{l s='Pack content'}</span></h3>
			{include file="$tpl_dir./product-list.tpl" products=$packItems class='slick'}
		</section>
		{/if}
		...
		{if isset($accessories) && $accessories}
			<!--Accessories -->
			<h3 class="title title_blue"><span class="title__text">{l s='Accessories'}</span></h3>
            {include file="$tpl_dir./product-list.tpl" products=$accessories class='slick'}
			<!--end Accessories -->
		{/if}
