...
class PRODUCTINFORMATION extends Module
{
	public function __construct()
	{
		$this->name = 'productinformation';
		$this->description = 'Информация о доставке';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Productinformation_MONOLANGUAGE';
		$this->text_editor_content = 'Productinformation_TEXTEDITOR';
		$this->content_wrapper = 'Productinformation_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Productinformation_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Productinformation_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('product-information');
		$this->description = $this->l('Информация о доставке');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !PRODUCTINFORMATIONModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !PRODUCTINFORMATIONModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = PRODUCTINFORMATIONModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = PRODUCTINFORMATIONModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					PRODUCTINFORMATIONModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					PRODUCTINFORMATIONModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																PRODUCTINFORMATIONModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class PRODUCTINFORMATIONModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( PRODUCTINFORMATIONModel::createFilesTable()
				&& PRODUCTINFORMATIONModel::createContentTable());
	}
